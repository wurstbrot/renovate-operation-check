...
class LintChecker:
    ...
    def lint_python_code(file_path):
        ...
        try:
            ...
            logger.info(f"Bandit security results for {file_path}: {'PASS' if bandit_result.returncode == 0 else 'FAIL'}")
            ...
            logger.info(f"Flake8 lint results for {file_path}: {'PASS' if flake8_result.returncode == 0 else 'FAIL'}")
            ...
            results["secrets"] = {
                "success": not has_secrets,
                "output": secrets_result.stdout
            }
            if has_secrets:
